...
.btn{border:none;padding:6px 14px;border-radius:6px;font-size:13px;font-weight:500;cursor:pointer;text-decoration:none;display:inline-flex;align-items:center}
...
.paper{background:#fff;box-shadow:0 4px 12px rgba(0,0,0,.1);border-radius:4px;display:flex;justify-content:center;align-items:center;overflow:hidden;width:200px;height:283px}
...
<header class="nav">
  <div style="display:flex;align-items:center;gap:16px">
    <div style="font-size:18px;font-weight:700">🖨️ CUPS 打印与扫描控制台</div>
    <div class="tab-group">
  ...
  <div style="display:flex;gap:10px">
    ...
    <a id="cupsL" target="_blank" class="btn btn-o">⚙️ 631管理后台</a>
  </div>
</header>
...
<section>
  <div class="card">
    <div class="card-h"><span>🖨️ 打印机与上传队列</span></div>
    <div class="fg" style="margin-bottom:16px"><select id="selP" class="fc" onchange="syncP()"></select></div>
    <div class="drop" id="dz"><input type="file" id="fi" multiple style="display:none"><div style="font-size:28px">📑</div><div>点击或拖入照片/PDF文件 (支持批量)</div></div>
...
<section>
  <div class="card">
    <div class="card-h"><span>📈 状态监控</span></div>
    <div class="st-row"><span>打印机状态</span><span class="badge badge-idle" id="stB">检测中</span></div>
...
<section>
  ...
  <div class="card">
    ...
    <div class="prev-box" style="min-height:300px">
      <div class="paper" id="scanPaperSheet" style="width:220px;height:310px">
...
const dz=document.getElementById('dz'), fi=document.getElementById('fi'), flist=document.getElementById('flist');
...
async function submitPrint(){
  if(!filesArr.length) return alert('请先选择文件！');
    ...
    def post(self):
        ...
        try:
            ...
            if res.returncode != 0 or not os.path.exists(raw) or os.path.getsize(raw) == 0:
                err_msg = res.stderr.strip() or res.stdout.strip() or "扫描仪未输出有效图像或连接中断"
