...
<head>
    ...
    <style>
        ...
        h2 { margin-top: 0; color: #004b87; font-size: 18px; }
        .btn { display: block; width: 100%; box-sizing: border-box; background: #004b87; color: white; border: none; padding: 12px; border-radius: 8px; font-size: 16px; cursor: pointer; text-align: center; text-decoration: none; margin-top: 10px; font-weight: bold; }
...
<body>
    ...
    <div class="card">
        <h2>⚙️ 底层管理后台</h2>
        <a href="javascript:void(0);" onclick="location.href='http://'+location.hostname+':631'" class="btn btn-gray">进入 CUPS 631 汉化后台</a>
    </div>
...
class ScanHandler(tornado.web.RequestHandler):
    def post(self):
        ...
        if ret.returncode == 0 and os.path.exists(pdf_path) and os.path.getsize(pdf_path) > 0:
            self.write("<script>alert('扫描完成！文件已保存');location.href='/scans/';</script>")
        else:
...
if __name__ == "__main__":
    ...
    print(f" [Web App] 快速打印/扫描控制台已监听 0.0.0.0:{PORT} ...", flush=True)
