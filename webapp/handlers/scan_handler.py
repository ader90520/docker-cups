...
class ScanHandler(BaseHandler):
    def get(self):
        """探测可用的扫描仪设备（全面支持 hpaio、sane、genesys 等）"""
        try:
    ...
    def post(self):
        """执行硬件扫描任务（采用稳健的无损重定向管道读取，杜绝底层 I/O 错误）"""
        try:
            ...
            if not device:
                chk = subprocess.run(["scanimage", "-L"], stdout=subprocess.PIPE, text=True, env=env)
            ...
            if not device:
                self.write_json(False, "未找到可用的一体机扫描设备，请确认设备已开机连接！")
                return
            ...
            cmd = ["scanimage", "-d", device, "--resolution", str(resolution), "--mode", mode]
            print(f"[ScanHandler] 执行扫描管道: {' '.join(cmd)}")
            ...
            if res.returncode != 0 or not os.path.exists(tmp_pnm) or os.path.getsize(tmp_pnm) == 0:
                err = res.stderr.decode("utf-8", errors="ignore").strip() or "扫描仪响应异常"
            ...
            self.write_json(False, "扫描仪响应超时，请检查设备是否发生卡纸或未闭合盖板！")
        except Exception as e:
