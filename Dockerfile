...
# 2. 预下载惠普全系列热门 GDI 固件并打包封入
RUN mkdir -p /usr/share/foo2zjs/firmware && \
    cd /tmp && \
...
# 4. 复制启动脚本、云打印脚本与汉化资产（匹配实际路径 i18/zh_CN/zh_CN/）
...
# 5. 彻底汉化：编译 mo 并将中文模板与中文首页覆盖到系统生效目录
